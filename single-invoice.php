...
if (isset($_GET['rendering']) && $_GET['rendering'] == 'pdf') {
    include_once(WPIM_PATH_INCLUDES . '/printable-pdf-invoice.php');
} else {

$wpim_settings = get_option('wpim_settings');
$invoice_page = isset($wpim_settings['invoice_page']) ? $wpim_settings['invoice_page'] : '';
$invoice_link = get_permalink($invoice_page) . '?uniid=' . get_post_meta($post_id, 'unique_key', true);
...
<div class="wpim-container">
    <div id="wpinvoicer">
        <div class="section-row">
            ...
            <form method="POST" @submit="saveInvoice($event)" id="wpim-form">
                ...
                <div class="content-half tools-part">
                    ...
                    <div class="form-actions">
                        <button class="btn button" style="width: 100%;display:block; margin:3px">Save </button>

           
                     <a href="<?= $invoice_link ?>&render=preview" target="_blank" class="btn button grey" style="width: 100%;display:block">Preview</a>
                     <a href="#" class="btn button grey" @click.prevent="generateInvoice()" style="width: 100%;display:block">Generate PDF</a>

                     <a href="#" @click.prevent="emailInvoice(<?= $post_id ?>)" class="btn button grey" style="width: 100%;display:block">Email Invoice</a>
                    </div>
            ...
            <!-- generate pdf form -->
            <form action="" method="post" target="_blank" id="generate-invoice-form">
                <input type="hidden" name="invoice_id" value="<?= $post_id ?>" />
